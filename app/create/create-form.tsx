...
export function CreateForm() {
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      if (!response.ok) return alert("Failed to get pre-signed URL.")
      ...
      if (!uploadResponse.ok) return alert("s3 Upload failed.")

      const indexFacesResponse = await fetch("/api/rekognition/index-faces", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (indexFacesResponse.ok) {
        ...
        const indexFacesResult: IndexFacesResponse = _indexFacesResult.response
